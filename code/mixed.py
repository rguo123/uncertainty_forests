...
def Mixed_KSG(x, y, k=5):
    ...
    knn_dis = [tree_xy.query(point, k+1, p=float('inf'))[0][k]
               for point in data]
    ans = 0
    ...
    for i in range(N):
        ...
        if knn_dis[i] == 0:
            ...
            nx = len(tree_x.query_ball_point(
                x[i], knn_dis[i]-1e-15, p=float('inf')))
            ny = len(tree_y.query_ball_point(
                y[i], knn_dis[i]-1e-15, p=float('inf')))
...
def KSG(x, y, k=5):
    ...
    for i in range(N):
        nx = len(tree_x.query_ball_point(
            x[i], knn_dis[i]+1e-15, p=float('inf')))-1
        ny = len(tree_y.query_ball_point(
            y[i], knn_dis[i]+1e-15, p=float('inf')))-1
        ans += (digamma(k) + log(N) - digamma(nx) - digamma(ny))/N
    return ans
